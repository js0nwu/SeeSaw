...
def initialize_environment():
    ...
    global GROUP_FREQUENCY
    cmd = r'find "' + TARGET_DIR + r'" -name "*"' + GROUP_FREQUENCY + r'"*ssensorData.csv" | grep -v "noise" | grep "' + ACTIVITY + r'"'
    import subprocess
    csvSensorDataFiles = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE).stdout.read().decode('utf-8').split('\n')[:-1]
    csvSensorDatas = []
    for f in csvSensorDataFiles:
...
pplot.savefig(TARGET_DIR + folderName + "_correlation_fp_threshold_precision" + "_" + sys.argv[4] + "_" + sys.argv[6] + str(SKIP_CYCLES).replace(".", "_") + "_" + ("saw" if FIT_SAW else "sine") + FILE_FORMAT)
