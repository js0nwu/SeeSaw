...
def initialize_environment():
    ...
    global GROUP_FREQUENCY
    cmd = r'find "' + TARGET_DIR + r'" -name "*"' + GROUP_FREQUENCY + r'"*ssensorData.csv" | grep -v "noise" | grep "' + ACTIVITY + r'"'z
    import subprocess
    csvSensorDataFiles = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE).stdout.read().decode('utf-8').split('\n')[:-1]
    csvSensorDatas = []
    for f in csvSensorDataFiles:
    ...
    csvNoiseSensorDatas = []
    cmd = r'find "' + TARGET_DIR + r'" -name "*"' + GROUP_FREQUENCY + r'"*ssensorData.csv" | grep "noise" | grep "' + ACTIVITY + r'"'
    csvSensorDataFiles = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE).stdout.read().decode('utf-8').split('\n')[:-1]
    for f in csvSensorDataFiles:
        csvSensorData = pd.read_csv(f, names=['time','sensor','x','y','z','timestamp','linecounter',''])
...
for sdi in range(len(csvSensorDatas)):
    ...
    discardNum = SKIP_CYCLES 
    # totalTime = int(leftEvents[-1] - (leftEvents[0] + discardNum*signalPeriod))
    totalTime = int(GROUP_FREQUENCY) * 15
    # waveLengths = [] 
    counter = 0
    for rowI in range(0, totalTime, totalTime // 15):
        print("window length", rowI, "file number", sdi)
        counter += 1
        startWindow = leftEvents[0] + discardNum*signalPeriod
        endWindow = leftEvents[0] + rowI
        window = []
        ...
        for thresh in thresholds:
            wKey = (thresh, rowI)
            if wKey not in dtcounter:
                dtcounter[wKey] = 0
            if wKey not in dttotal:
                dttotal[wKey] = 0
            if cc[0] > thresh:
                dtcounter[wKey] += 1
            dttotal[wKey] += 1
...
# precisions = []
# for wKey in dtcounter:
#     correct = dtcounter[wKey]
#     total = dttotal[wKey]
#     precision1 = float(correct) / float(total)
#     correct = dfcounter[wKey]
#     total = dftotal[wKey]
#     precision2 = float(correct) / float(total)
#     accuracy = 0.5 * (precision1 + precision2)
#     precisions.append([wKey[0], wKey[1], accuracy])
...
for wKey in dtcounter:
    # if wKey[0] >= 0.9:
    #     continue
    correct1 = dtcounter[wKey]
    total1 = dttotal[wKey]
    diff = 0
    # w2 = None
    w2 = wKey
    # for wKey2 in dfcounter:
    #     if wKey2[0] == wKey[0]:
    #         if w2 is None or abs(wKey2[1] - wKey[1]) < diff:
    #             w2 = wKey2
    #             diff = abs(wKey2[1] - wKey[1])
    correct2 = dfcounter[w2]
    total2 = dftotal[w2]
    # this is not correct
    # accuracy = (correct1 + correct2) / (total1 + total2)
    true_positives = correct1 / total1
    true_negatives = correct2 / total2
    print("tp", true_positives, "tn", true_negatives)
    accuracy = (true_positives + true_negatives) / 2 # assuming equal true positives and true negatives
    precisions.append([wKey[0], wKey[1], accuracy])
...
# dtcountern = np.array([[i[0][0], i[0][1], i[1]] for i in dtcounter.items()])
# dfcountern = np.array([[i[0][0], i[0][1], i[1]] for i in dfcounter.items()])
# xmin = max(dtcountern[:,0].min(), dfcountern[:,0].min())
# xmax = min(dtcountern[:,0].max(), dfcountern[:,0].max())
# ymin = max(dtcountern[:,1].min(), dfcountern[:,1].min())
# ymax = min(dtcountern[:,1].max(), dfcountern[:,1].max())
# xi = np.linspace(xmin, xmax)
# yi = np.linspace(ymin, ymax)
# xg, yg = np.mgrid[xmin:xmax:100j, ymin:ymax:100j]
# dtcounteri = interp.griddata(dtcountern[:,0:2], dtcountern[:,2], (xg.ravel(), yg.ravel()), method='linear')
# dfcounteri = interp.griddata(dfcountern[:,0:2], dfcountern[:,2], (xg.ravel(), yg.ravel()), method='linear')
# dtcounteri = mlab.griddata(dtcountern[:,0], dtcountern[:,1], dtcountern[:,2], xi, yi, interp='linear').data
# dfcounteri = mlab.griddata(dfcountern[:,0], dfcountern[:,1], dfcountern[:,2], xi, yi, interp='linear').data

# dtcounteri = interp.griddata(dtcountern[:,0:2], dtcountern[:,2], (xg, yg), method='linear')
# dfcounteri = interp.griddata(dfcountern[:,0:2], dfcountern[:,2], (xg, yg), method='linear')
# precisions = (dtcounteri + dfcounteri) / 2
...
# ax.set_zlim3d(np.min(Z), np.max(Z))
# fig.colorbar(surf)
# contour = ax.contour(xd, yd, zd, cmap=cm.coolwarm)
contour = ax.contourf(X, Y, Z, np.linspace(0.0, 1.0, 21, endpoint=True), cmap=cm.coolwarm)
# contour = ax.contourf(X, Y, precisions, np.linspace(0.0, 1.0, 21, endpoint=True), cmap=cm.coolwarm)
# pplot.clim(0.0, 1.0)
...
pplot.savefig(TARGET_DIR + folderName + "_correlation_threshold_precision" + "_" + sys.argv[4]  + "_" + sys.argv[6] + str(SKIP_CYCLES).replace(".", "_") + "_" + ("saw" if FIT_SAW else "sine") + FILE_FORMAT)