...
def data_lags(csvData):
    ...
    discardNum = SKIP_CYCLES 
    windowLength = 4000
    totalTime = int(leftEvents[-1] - (leftEvents[0] + windowLength))
    waveLengths = [] 
    counter = 0
    for rowI in range(0, totalTime, totalTime // 150):
        counter += 1
        startWindow = leftEvents[0] + rowI
        endWindow = leftEvents[0] + rowI + windowLength
        window = []
        for v in valueS:
            valueTime = v[0]
            if valueTime >= startWindow and valueTime <= endWindow:
        ...
        lag_factor = find_timeshift(rs2, ps2)
        lag_time = resample_time * lag_factor
        correlationCoefficients.append(abs(lag_time))
    correlationCoefficients = np.array(correlationCoefficients)
...
def data_correlation_graphs3_noise(csvData):
    ...
    waveLengths = [] 
    counter = 0
    for rowI in range(0, totalTime, totalTime // 100):
        counter += 1
...
for freq in frequencies:
    ...
    fcount = 0
    for f in csvSensorDatas:
        fcount += 1
        print("file", fcount, "/", len(csvSensorDatas))
        lcount, llags = data_lags(f)
        for l in llags:
            lag_factors.append(l)

    lag_factors = np.array(lag_factors)
    f_lags.append(np.mean(lag_factors))
...
f_lags = [abs(f) for f in f_lags]

y_pos = np.arange(len(frequencies))

pplot.clf()
pplot.bar(y_pos, f_lags, align='center')
pplot.xticks(y_pos, frequencies)
pplot.xlabel("Frequency (ms)")
pplot.ylabel("Average Adjustment (ms)")

folderName = sys.argv[1].split("/")[-1]
print("folder name:", folderName)
pplot.savefig(TARGET_DIR + folderName + "_correlation_threshold_precision" + str(SKIP_CYCLES).replace(".", "_") + "_" + ("saw" if FIT_SAW else "sine") + FILE_FORMAT)