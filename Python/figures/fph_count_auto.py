...
def test_file(filepath, thresholds):
    with open(os.devnull, "w") as devnull:
        result = subprocess.run(["java", "-jar", "javaapp-offlinetester.jar", filepath, "-1", "1"], stdout=subprocess.PIPE, stderr=devnull)
        ...
        if "\n" in result_string:
            for threshold in thresholds:
                ...
                for line in lines:
                    ...
                    direction = row[3]
                    if correlation >= threshold and timestamp - last_sync > SYNC_LOCK:
