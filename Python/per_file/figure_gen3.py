...
def data_plot_raw_data(draw_feature=True):
    ...
    for rowI in range(len(csvData['sensor'])):
        ...
        if row == PLOT_SENSOR:
            ...
            ax1.axvline(x=adjustX, color='red')
        elif row == 'right':
            rx = x
            ry = y
            rz = z
            ...
            adjustX = int(csvData['time'][rowI]) - min(times)
            ax1.axvline(x=adjustX, color='black')
        elif row == 'vibrate':
            adjustX = int(csvData['time'][rowI] - min(times))
            ax1.axvline(x=adjustX, color='green')
    ...
    ax1.plot(plotX[:,0] - np.min(plotX[:,0]), get_ewma(plotX[:,1], alphaFactors[PLOT_SENSOR]), color='red')
    ax1.plot(plotY[:,0] - np.min(plotY[:,0]), get_ewma(plotY[:,1], alphaFactors[PLOT_SENSOR]), color='green')
    ax1.plot(plotZ[:,0] - np.min(plotZ[:,0]), get_ewma(plotZ[:,1], alphaFactors[PLOT_SENSOR]), color='blue')
    ax1.plot(plotMag[:,0] - np.min(plotMag[:,0]), get_ewma(plotMag[:,1], 1), color='turquoise')
    # ax1.plot(plotT[:,0] - np.min(plotT[:,0]), get_ewma(plotT[:,1], alphaFactors[PLOT_SENSOR]), color="magenta")
    if draw_feature:
        ax1.plot(plotTL[:,0] - np.min(plotTL[:,0]), get_ewma(plotTL[:,1], alphaFactors[PLOT_SENSOR]), color='purple')
    pplot.savefig(generate_filename("raw_data_tl")) if SAVE_FILES else pplot.show()
...
def data_lag_calculation():
    ...
    for rowI in range(startIndex, endIndex):
        ...
        if row == PLOT_SENSOR:
            ...
            pplot.axvline(x=csvData['time'][rowI] - np.min(np.array(leftEvents)), color='red')
        elif row == 'right':
            rightEvents.append(csvData['time'][rowI])
            pplot.axvline(x=csvData['time'][rowI] - np.min(np.array(rightEvents)), color='black')
    plotX = np.array(plotX)
    plotY = np.array(plotY)
    plotZ = np.array(plotZ)
    plotMag = np.array(plotMag)
...
def data_correlation_graphs():
    ...
    for rowI in range(0, totalTime, totalTime // 150):
        ...
        if SHOW_CORRELATION_WINDOWS:
            ...
            ax1.plot(window[:,0] - np.min(window[:,0]), actualRange * np.array(referenceSignal(window[:,0])) + np.mean(actualSignal), color="green", linestyle="dotted")
            ax1.plot(window[:,0] - np.min(window[:,0]), actualRange * generatedSignal + np.mean(actualSignal), color="green")
            ax1.plot(window[:,0] - np.min(window[:,0]), actualSignal, color="purple")
            for e in leftEvents:
                if e >= startWindow and e <= endWindow:
                    ax1.axvline(x=e - np.min(window[:,0]), color="red")
            for e in rightEvents:
                if e >= startWindow and e <= endWindow:
                    ax1.axvline(x=e - np.min(window[:,0]), color="black")
    ...
    pplot.savefig(generate_filename("correlate_window_summary_tl")) if SAVE_FILES else pplot.show()
...
def data_3d_plot():
    ...
    pplot.show()
    # pplot.savefig(generate_filename("3d_vector_plot")) if SAVE_FILES else pplot.show()

def data_3d_animation():
    csvData = csvSensorData
    plotX = []
    plotY = []
    plotZ = []
    ...
    pvectors = np.array(pvectors)
    X2,Y2,Z2,U2,V2,W2 = zip(*pvectors)
    ax.quiver(X2,Y2,Z2,U2,V2,W2, pivot="middle", arrow_length_ratio=0.05,color="red")
    ax.set_xlim([-1,1])
    ax.set_ylim([-1,1])
    ax.set_zlim([-1,1])
    pplot.savefig(generate_filename("3d_vector_plot")) if SAVE_FILES else pplot.show()
