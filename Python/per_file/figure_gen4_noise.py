...
def initialize_environment():
    ...
    csvNoiseSensorDataFiles = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE).stdout.read().decode('utf-8').split('\n')[:-1]
    csvNoiseSensorDatas = []
    ...
    for f in csvNoiseSensorDataFiles:
        csvNoiseSensorData = pd.read_csv(f, names=['time','sensor','x','y','z','timestamp','linecounter',''])
        csvNoiseSensorDatas.append(csvNoiseSensorData)
...
def data_correlation_graphs2(csvData):
    ...
    maxCorrelation = np.max(correlationCoefficients[:,0][~np.isnan(correlationCoefficients[:,0])])
    print(np.argmax(correlationCoefficients[:,0][~np.isnan(correlationCoefficients[:,0])]))
    print(maxCorrelation)
    return maxCorrelation
def plot_category(category, category_color):
    global GROUP_FREQUENCY
    GROUP_FREQUENCY = category
    initialize_environment()
    mcs = []
    for sdi in range(len(csvNoiseSensorDatas)):
        sd = csvNoiseSensorDatas[sdi]
        mc = data_correlation_graphs2(sd)
        mcs.append(mc)

    thresholds = np.arange(-1, 1, 0.1)
    accepts = [0 for i in thresholds]

    for threshi in range(thresholds.size):
        for mci in range(len(mcs)):
            mc = mcs[mci]
            thresh = thresholds[threshi]
            if mc > thresh:
                accepts[threshi] += 1
    accepts = [float(a) / float(len(mcs)) for a in accepts]
    pplot.plot(thresholds, np.array(accepts), color=category_color)

frequencies = [("750", "red"), ("1000", "green"), ("1250", "blue")]
for f in frequencies:
    plot_category(f[0], f[1])
import os
folderName = os.getcwd().split("/")[-1]
pplot.savefig(TARGET_DIR + folderName + "_threshold_graph_noise" + str(SKIP_CYCLES).replace(".", "_") + FILE_FORMAT)