...
FIT_SAW = False if (len(sys.argv) == 3 or sys.argv[3] == "False") else True 
...
def data_correlation_graphs2_mag(csvData, use_saw):
    valueX = []
    valueY = []
    valueZ = []
    valueMag = []
    ...
    referenceSignal = lambda x : signal.sawtooth((2*np.pi/signalPeriod)*x + (1.57079633 - (2*np.pi/signalPeriod)*leftEvents[0]), width=0.5) if use_saw else np.sin((2*np.pi/signalPeriod)*x + (1.57079633 - (2*np.pi/signalPeriod)*leftEvents[0])) # arcsin(1)
    ...
    for rowI in range(0, totalTime, totalTime // 150):
        ...
        generatedSignal = signal.sawtooth((2*np.pi/signalPeriod)*signal_range + (1.57079633 - (2*np.pi/signalPeriod)*(leftEvents[0]+lag_time)), width=0.5) if use_saw else np.sin((2*np.pi/signalPeriod)*signal_range + (1.57079633 - (2*np.pi/signalPeriod)*(leftEvents[0]+lag_time))) # arcsin(1)
        if CORRELATION_EWMA:
...
def data_correlation_graphs2(csvData, use_saw):
    valueX = []
    valueY = []
    valueZ = []
    valueMag = []
...
def plot_category(ax, category, category_color):
    ...
    for sdi in range(len(csvSensorDatas)):
        ...
        mc = data_correlation_graphs2(sd, True)
        mcs.append(mc)
    ...
    for sdi in range(len(csvSensorDatas)):
        sd = csvSensorDatas[sdi]
        mc2 = data_correlation_graphs2(sd, False)
        mcs2.append(mc2)

    minLength = min([i.shape[0] for i in mcs2])
    mcs2 = [cc[:minLength] for cc in mcs2]
    avgCCs2 = np.array(mcs2).mean(axis=0)
    ax1.plot(avgCCs[:,0], avgCCs[:,1] - avgCCs2[:,1], color=category_color)

def plot_category_mag(ax, category, category_color):
    global GROUP_FREQUENCY
    GROUP_FREQUENCY = category
    initialize_environment()
    mcs = []
    for sdi in range(len(csvSensorDatas)):
        sd = csvSensorDatas[sdi]
        mc = data_correlation_graphs2_mag(sd, True)
        mcs.append(mc)
    ...
    for sdi in range(len(csvSensorDatas)):
        sd = csvSensorDatas[sdi]
        mc2 = data_correlation_graphs2_mag(sd, False)
        mcs2.append(mc2)

    minLength = min([i.shape[0] for i in mcs2])
    mcs2 = [cc[:minLength] for cc in mcs2]
    avgCCs2 = np.array(mcs2).mean(axis=0)
    ax1.plot(avgCCs[:,0], avgCCs[:,1] - avgCCs2[:,1], color=category_color, linestyle="dotted")
...
for freq in frequencies:
    ...
    plot_category_mag(ax1, freq[0], freq[1])
    

folderName = sys.argv[1].split("/")[-1]
print("folder name:", folderName)
pplot.savefig(TARGET_DIR + folderName + "_correlation_time_avg" + str(SKIP_CYCLES).replace(".", "_") + "_diff" + FILE_FORMAT)