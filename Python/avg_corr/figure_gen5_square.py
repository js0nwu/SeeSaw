...
def plot_category_mag(ax, category, category_color):
    ...
    for sdi in range(len(csvSensorDatas)):
        sd = csvSensorDatas[sdi]
        mc = data_correlation_graphs2_mag(sd)
        mcs.append(mc)
...
pplot.savefig(TARGET_DIR + folderName + "_correlation_time_avg" + str(SKIP_CYCLES).replace(".", "_") + "_" + ("square" if FIT_SQUARE else "sine") + FILE_FORMAT)