 ...
 */
public class StdTapDetector extends Detector<Tuple2<NVector>> {
    ...
    private float tupleStat(Tuple2<Float>[] tupleList) {
        ...
        float[] diffs = new float[aList.size()];
        for (int i = 0; i < aList.size(); i++) {
            diffs[i] = aList.get(i) - bList.get(i);
        }
        NVector diffVector = new NVector(diffs);
        return diffVector.getStd();
    }
    ...
    public boolean detect(Tuple2<NVector> data) {
        ...
        if (received < TAP_WINDOW_SIZE) {
            ...
            System.out.println("std tap detector: " + "qMag " + queryMag + " rMeanMag " + resultsMeanMag + " rStdMag " + resultsMeanStd + " stdDistanceMag " + stdDistanceMag + " pass " + ((stdDistanceMag > TAP_ACCEPT_THRESHOLD) ? "True" : "False"));
    ...
    public String detectDirection(Tuple2<NVector> data) {
        ...
        System.out.println("stdtapdetector: " + direction);
        return direction;
    }
}
